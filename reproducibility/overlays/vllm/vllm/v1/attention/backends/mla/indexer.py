...
#
# R17 port: pristine v0.27.1 indexer.py used as the base (patched full copy;
# see PORTS_B12X_NOTES.md for the strategy decision), with the R9 B12X-specific
# additions re-applied:
#   (a) block-table width 128-element alignment (_align_indexer_block_table_width)
#   (b) decode_topk_max_seq_len host-sync removal (max_seq_len on the decode
#       metadata is filled from the host-scalar common_attn_metadata.max_seq_len
#       on the B12X path; the device-tensor `seq_lens.max().item()` reduction is
#       never performed)
#   (c) B12X sparse-indexer decode/prefill support (schedule metadata,
#       active-width window, per-request supertile prefill chunking,
#       B12X_NON_COMPRESSED_INDEXER backend alias)
import os
...
# --- B12X sparse-indexer support (R9 port) -------------------------------
# R9 read envs.VLLM_USE_B12X_SPARSE_INDEXER; that knob is not defined in the
# pristine 0.27.1 vllm/envs.py (the envs port is a separate PORT_MAP row), so
# read the process environment directly to keep this file self-contained.
...
def _align_indexer_block_table_width(
    max_num_blocks_per_req: int,
    block_size: int,
) -> int:
    """Match the 128-element alignment used by MultiGroupBlockTable.

    R9 fix (a): the expanded block-table buffer must be at least as wide as
    the runner's actual block table, whose width is padded to a 128-element
    multiple. Applied defensively to the injected ``block_table_width``; a
    no-op when the caller already passes the aligned width.
    """
    if block_size <= 128:
        alignment = 128 // block_size
        return (
            (max_num_blocks_per_req + alignment - 1) // alignment
        ) * alignment
...
class DeepSeekV32IndexerDecodeMetadata:
    ...
    # None on the B12X path when the b12x paged-MQA schedule is not used.
    schedule_metadata: torch.Tensor | None
    global_seq_lens: torch.Tensor | None = None
    # --- B12X additions (R9 port) ---
    # Host-scalar upper bound on the max (uncompressed, global) context across
    # the batch. R9 fix (b): filled from common_attn_metadata.max_seq_len (a
    # host scalar) instead of `seq_lens.max().item()` (a blocking D2H sync on a
    # device tensor). 0 when not on the B12X path (no pristine consumer reads
    # it).
    max_seq_len: int = 0
...
class DeepseekV32IndexerMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(self, *args, block_table_width: int, **kwargs) -> None:
        ...
        # R9 fix (a): align the expanded block-table width to the 128-element
        # multiple used by MultiGroupBlockTable, so expanded rows can always
        # receive a full copy of a runner block-table row.
        block_table_width = _align_indexer_block_table_width(
            block_table_width, self.kv_cache_spec.block_size
        )
    ...
    def _maybe_build_b12x_schedule_metadata(
        ...
        if not _USE_B12X_SPARSE_INDEXER or requires_padding:
            return None
    ...
    def build(
        ...
        if num_prefills > 0:
            ...
            assert common_attn_metadata.seq_lens_cpu_upper_bound is not None
            seq_lens_cpu = common_attn_metadata.seq_lens_cpu_upper_bound
            if _USE_B12X_SPARSE_INDEXER:
                # B12X paged-index prefill scores in fixed K supertiles; chunk
                # each prefill request independently against the supertile
                # budget so a single long request never fuses with others.
                chunk_specs = []
        ...
        if num_decodes > 0:
            ...
            schedule_metadata: torch.Tensor | None = self.scheduler_metadata_buffer
            if _USE_B12X_SPARSE_INDEXER:
            ...
            decode_topk_max_seq_len = 0
            active_width = None
            if _USE_B12X_SPARSE_INDEXER:
                # R9 fix (b): never reduce the device-resident `seq_lens` on
                # the host (`.max().item()` would block every decode build).
                # common_attn_metadata.max_seq_len is already a host scalar and
                # a conservative upper bound (global and uncompressed, where
                # `seq_lens` may be DCP-local and/or divided by
                # compress_ratio).
                decode_topk_max_seq_len = int(common_attn_metadata.max_seq_len)
...
def build_prefill_chunk_metadata(
    ...
    # B12X DCP global top-k requires every rank to produce a chunk (collective
    # participation), even when this rank owns zero KV for the range.
