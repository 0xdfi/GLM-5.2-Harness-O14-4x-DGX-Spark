...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def execute_model(
        ...
        self.execute_model_state = ExecuteModelState(
            ...
            # R9/r15 port: SchedulerOutput.num_spec_tokens_to_schedule (added
            # by the scheduler lane's port) is 0 for synthetic/warmup outputs
            # and for schedules that never populate it; fall back to the
            # speculator's fixed configured depth in that case (matches
            # r9-payload's `resolve_num_spec_tokens_to_schedule()` behavior,
            # which was not ported onto SchedulerOutput in this pass -- see
            # PORTS_SCHED_NOTES.md Assumption A1).
    ...
    def sample_tokens(
        ...
        if self.speculator is not None:
            ...
            # R9/r15 port: defensive re-slice to the scheduler-selected depth
            # in case a speculator implementation ignores the
            # num_speculative_tokens kwarg and always returns full-depth
            # draft tokens (see spec_decode/utils.py:limit_draft_tokens).
            draft_tokens = limit_draft_tokens(
                draft_tokens,
                num_spec_tokens_to_schedule,
                self.num_speculative_steps,
            )
            # req_states.draft_tokens is a fixed-width [max_num_reqs,
            # num_speculative_steps] buffer; only refresh the first
            # num_draft_tokens columns so a narrowed depth doesn't leave
            # stale/garbage trailing columns silently reused downstream.
...
class ExecuteModelState(NamedTuple):
    ...
    # Speculative depth chosen by the scheduler for this step (R9/r15 port),
    # carried across the execute_model / sample_tokens boundary so the draft
    # loop can shorten below the speculator's configured max depth.
