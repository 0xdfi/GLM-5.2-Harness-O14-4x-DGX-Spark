...
class AutoRegressiveSpeculator(DraftModelSpeculator):
    ...
    def propose(
        ...
        # [max_num_reqs]
        seeds: torch.Tensor,
        # Scheduler-selected speculative depth for this step (R9/r15 port).
        # None (or the static/no-dynamic-SD default) means "use the
        # configured max depth", matching pre-port behavior exactly.
        ...
        # Generate the remaining num_speculative_tokens - 1 draft tokens.
    ...
    def _multi_step_decode(
        ...
        # R9/r15 port: use the per-call scheduler-selected depth
        # (`num_speculative_tokens`) as the loop bound instead of the
        # speculator's fixed configured max (`self.num_speculative_steps`),
        # so a step count below the ladder maximum does the correct amount
        # of decode work instead of always drafting to full depth.
        if num_speculative_tokens is None:
            num_speculative_tokens = self.num_speculative_steps
