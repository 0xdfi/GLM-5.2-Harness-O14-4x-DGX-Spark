...
def _assert_dcp1_for_b12x(dcp_world_size: int) -> None:
    if dcp_world_size > 1:
        raise NotImplementedError(
            "B12X sparse indexer DCP>1 merge is not ported in this R17 build "
            "(R17 v1 is DCP1); got decode_context_parallel_size="
            f"{dcp_world_size}. Disable VLLM_USE_B12X_SPARSE_INDEXER or run "
            "DCP1, or fall back to the upstream DeepGEMM indexer path."
        )
...
class SparseAttnIndexer(CustomOp):
    ...
    def __init__(
        ...
        if _b12x_sparse_indexer_requested():
            ...
            # forward pass.
            _ensure_b12x_sparse_indexer_supported()
            _assert_dcp1_for_b12x(self.dcp_world_size)
