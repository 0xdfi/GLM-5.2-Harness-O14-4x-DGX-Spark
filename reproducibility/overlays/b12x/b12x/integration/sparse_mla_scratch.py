...
class B12XSparseMLAScratchCaps:
    ...
    def __post_init__(self) -> None:
        ...
        kv_cache_dtype = (
            None if self.kv_cache_dtype is None else str(self.kv_cache_dtype)
        )
        scale_format = None if self.scale_format is None else int(self.scale_format)
        if kv_cache_dtype == "nvfp4_ds_mla" and scale_format is None:
            scale_format = 2
        if scale_format == 2:
            if self.kv_dtype is not torch.uint8:
                raise TypeError(
                    "NVFP4 sparse MLA scratch requires kv_dtype=torch.uint8"
                )
            if kv_cache_dtype not in (None, "nvfp4_ds_mla"):
                raise ValueError(
                    "scale_format=2 is only valid for "
                    "kv_cache_dtype='nvfp4_ds_mla'"
                )
        object.__setattr__(self, "kv_cache_dtype", kv_cache_dtype)
        object.__setattr__(self, "scale_format", scale_format)
