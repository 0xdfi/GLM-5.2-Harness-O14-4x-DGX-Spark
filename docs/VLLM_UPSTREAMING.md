...
**No**. The **Public O14 source / evidence** column is also a source-closure
boundary: it names repository-relative public files and symbols where available
and explicitly says when implementation or targeted evidence is not published.
...
|---|---|---|---|---|---|---|
| **vLLM 0.27 custom-port shell** | vLLM-derived source; 0.27 re-port and reconciliation by 0xdfi/Nous; individual features retain the donor lineages below. | Public boundary/evidence: [port lineage](PORT_LINEAGE.md), [custom-runtime inventory](CUSTOM_RUNTIME.md), [serve recipe](../recipe/serve-o14.sh), and [runtime record](../evidence/o14-results.json). The complete pre-O14 base is **not published**. Neither the campaign's consolidated 26-file release count nor a separate audit's 27-Python-file count is a reproducible public patch manifest; the counts used different scopes, and their scope reconciliation is unpublished. | **Yes**, as the runtime shell. The campaign's consolidated release count is 26 files; a separate audit counted 27 Python files under a different scope. | Not a comparable feature on main. The old APIs and the frozen main have diverged, so a wholesale file copy would be stale. | **No** monolithic PR. Decompose and reimplement only current defects. | Exact old base diff, a reproducible public patch manifest, published 26/27-scope reconciliation, and upstream-style tests. |
| **Scheduler async-preemption race repair** | Original 0xdfi/Nous correctness repair in a vLLM-derived scheduler. | The O14 scheduler implementation and targeted race trace are **not published** here; the [port-lineage reproduction boundary](PORT_LINEAGE.md#reproduction-boundary) records the omitted base. No public current-main reproducer or predecessor thread was verified. | **Yes.** The guarded request-index lookup was present; runtime survival is supporting evidence, not a targeted race proof. | **Unresolved:** scheduler structure changed, and the audit did not establish that the race still reproduces in the [frozen scheduler](https://github.com/vllm-project/vllm/blob/fe4c5dcd4cc71c2c1dc05375be497e413425aad7/vllm/v1/core/sched/scheduler.py). | **New PR** only after current-main reproduction. | Minimal failing unit test, current-main RED/GREEN proof, concurrency stress test, and proof that request snapshots do not already close the race. |
...
| **DFlash, DCP/CP, and structured-output compatibility** | Primarily vLLM-derived 0.27 API re-anchoring by 0xdfi/Nous; several initial payloads were absent from the final port tree. | Public docs/config only: [port lineage](PORT_LINEAGE.md#rebuild-base), [custom-runtime inventory](CUSTOM_RUNTIME.md#functional-surfaces), and DCP1 in [`serve-o14.sh`](../recipe/serve-o14.sh). Final DFlash/DCP/structured-output port source is **not published**; DCP1 is not DCP>1 or DP evidence. | Structured-output compatibility was installed. DCP-specific behavior was inert at DCP1. DFlash/autoregressive paths were not the active proposer. | DFlash and structured output were superseded by current implementations and tests. DCP sparse-MLA support remained partial; related work included [`#47779`](https://github.com/vllm-project/vllm/pull/47779), [`#50382`](https://github.com/vllm-project/vllm/pull/50382), and [`#46514`](https://github.com/vllm-project/vllm/pull/46514). | **No** DFlash/structured-output port. **Hold** DCP unless a DCP>1 current-main defect is reproduced. No initial comments. | Public final patch boundaries, DCP>1 execution, branch-specific failing tests, and proof for the five initial-but-not-final payload files. |
| **Launch and benchmark packaging** | Original 0xdfi/Nous reconstruction recipe, probes, evidence packaging, and documentation. | Public artifacts: [`serve-o14.sh`](../recipe/serve-o14.sh), [`o14.env.example`](../recipe/o14.env.example), [`benchmarks/README.md`](../benchmarks/README.md), [`o14-results.json`](../evidence/o14-results.json), [`final-o14-battery.jsonl`](../evidence/final-o14-battery.jsonl), and the [benchmark protocol](BENCHMARKS.md). | The package describes the measured envelope, but it is not an exact executable record of the private runtime. | Not applicable to vLLM core. | **No** runtime PR. Retain out of tree as reproducibility documentation. | Public base/checkpoint/sidecar, captured resolved launch manifest, topology, and reconciliation of the documented-versus-observed `CUDA_DEVICE_MAX_CONNECTIONS` value. |
...
5. **Adaptive MTP — Watch; no immediate comment.** Reconsider `#47111` only if
   it reactivates and O14 has material public controller/evidence, including a
...
2. **WATCH / NO COMMENT NOW — [`vLLM#47111`](https://github.com/vllm-project/vllm/pull/47111):**
   the PR needs a rebase and is inactive, and O14 has neither a published
   controller nor fixed-K A/B. Reconsider only if the PR reactivates and O14 has
...
- The monolithic 0.27 port and private `B12X_MLA_SPARSE` integration.
- The private 368-byte `nvfp4_ds_mla` ABI until its format, implementation,
  tests, dependencies, and donor licenses are public and reconciled.
...
- Most private B12X backend identity/indexer plumbing and the core int64 repair;
  only independently reproduced current-main gaps merit new work.
...
Known licensing/provenance gaps remain: the exact installed B12X source receipt
was not retained, the audited danielwoz NVFP4 repository did not expose a
confirmed license, and some standalone development files lack per-file SPDX
headers. Those gaps are blockers for copying the affected donor code, not an
invitation to label it Nous-original.
...
- **Private dependencies:** the base image, checkpoint, full base port, and W8
  sidecar weights are not public. The package is not a stock-vLLM reproducer.
